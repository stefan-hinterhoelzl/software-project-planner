...
export async function removeRemoteIssuesFromProjectViewpoint(req: Request, res: Response) {
    ...
    try {
        ...
        const conn = await connect()
        await Promise.all([issues.map(value => conn.query('DELETE FROM RemoteIssues WHERE projectViewpointId = ? AND remoteProjectId = ? AND remoteIssueId = ?', [value.projectViewpointId, value.remoteProjectId, value.issueRemoteId]))])
